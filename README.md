...
### PDF Figure Cropping: `mineru-pdf-crop` and `paddle-pdf-crop`

- Post-processing based on advanced OCR tools: both MinerU and PaddleOCR outputs include `.json` files with layout information.
- Use case: when you need figures and tables from the original paper in `.pdf` format, for example to insert vector graphics into LaTeX Beamer.
- The core task is locating figure/table bounding boxes. Without advanced OCR as a prerequisite, an agent may try to use tools such as `PyMuPDF` to achieve something similar, but the localization accuracy is limited, and it is hard to precisely and robustly identify and extract accompanying titles, notes, and related information.
...
### Discussing and Presenting Literature: `/econ-pre`
...
> Here I only briefly explain the basic development approach, discuss how to use it (in other words, the boundary I am trying to draw), and introduce its functionality along the way (which is essentially the skills that receive its task routing). See [econ-pre-demo]()

Making a presentation is for undergraduates and master's students; for doctoral students,
